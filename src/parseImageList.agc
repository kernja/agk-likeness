...
rem assign a sprite an image
rem load image into memory if it is needed
function LazySetSpriteImage(pSprite, pImage)
    if GetImageExists(pImage) = 0
        LoadParsedImage(pImage)
    endif

    SetSpriteImage(pSprite, pImage)
endfunction
